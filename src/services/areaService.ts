...
export const getAreaById = async (id: number): Promise<Area> => {
    const result = await pool.query('SELECT * FROM Areas WHERE AreaID = $1', [id]);
    return result.rows[0];
};

export const createArea = async (name: string, description?: string, imageUrl?: string): Promise<Area> => {
...
export const updateArea = async (id: number, name: string, description?: string, imageUrl?: string): Promise<Area> => {
    const result = await pool.query(
